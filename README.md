...
- A snapshot saved within `min_save_interval` of the previous one is stored without scrollback on purpose (it keeps a restore storm's hook-per-window saves cheap); the snapshot picker's preview and the close picker's per-pane block both read `(scrollback skipped — saved within min_save_interval)` for its panes.
