...
- `--kind=close` — two columns: the grouped close tree on the left, and on the right a preview of what the row under the cursor would reopen — the window's pane map, or that pane's captured scrollback. It follows the cursor with no Tab, and needs no 120-column threshold (under ~80 columns it drops to the tree alone). The closed panes are read out of the snapshot the close was diffed against, which is where their scrollback comes from. Used by `prefix + U`.
